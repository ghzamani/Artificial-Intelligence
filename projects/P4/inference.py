...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        cumulative = 0
        for d in list(self.items()):
            if cumulative+d[1] > r:
                return d[0]
            cumulative += d[1]

        # for k in self.keys():
        #     if num + self.dist[k] > prob:
        #         return k
        #     num += self.dist[k]
        dist = self.copy()
        dist.normalize()
        ...
        # letters = []
        # letters.append('first_item')

        # for k in dist.keys():
        #     cumulative.append(cumulative[-1] + dist[k])
        #     letters.append(k)

        # r = random.random()
        # for i in range(0, len(cumulative) - 2):
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # iterate your updates over the variable self.allPositions
        for p in self.allPositions:
            prob = self.getObservationProb(observation, pac_pos, p, jail_pos)
            self.beliefs[p] = prob * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            # assume the ghost was in "oldPos"
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            for pos in newPosDist.keys():
                new_beliefs[pos] += newPosDist[pos] * self.beliefs[oldPos] 
